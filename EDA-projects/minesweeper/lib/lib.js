...
function showCell (evt) {
  ...
  if (evt.target.classList.contains('mine')) {
    ...
    console.log('Adding a w# class to div'+ evt.target.classList);
    evt.target.classList.add('w'+wnum);
    //////////////////////////////////////////////////////////
...
function revealMines () {
  makeArray(document.getElementsByClassName('mine'))
    .forEach(function (element) {
      ...
      let wnum = Math.floor(Math.random()*5);
      console.log('Adding a w# class to div'+ element.classList);
      element.classList.add('w'+wnum);
    })
}
